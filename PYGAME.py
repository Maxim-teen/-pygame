...
class MyApp(QObject):
    ...
    def __init__(self):
        ...
        self.user_score = 0
        self.username = ""  # Сохраняем имя пользователя для дальнейшего использования
    ...
    def start_game(self):
        # Запускаем игру в новом потоке, передавая сигнал счета и имя пользователя
    ...
    def update_score_display(self, score):
        self.user_score = score  # Обновляем счет пользователя
        if hasattr(self, 'menu_dialog'):
            self.menu_dialog.update_score(score)

    def save_user_progress(self):
        try:
            # Получаем текущий счет пользователя
            current_score = self.user_score
            # Обновляем счет в файле
            self.update_user_score(self.username, current_score)
            ...
            with open(USERS_FILE, 'r') as file:
                for line in file:
                    if line.startswith(username + ":"):
                        ...
                        user_data[3] = str(new_score)  # Обновление счета
                        line = ":".join(user_data) + "\n"
                    rows.append(line)
            ...
            if os.path.exists(USERS_FILE):
                with open(USERS_FILE, 'r') as file:
                    for line in file:
                        if line.startswith(username + ":"):
                            user_data = line.strip().split(":")
                            if user_data[2] == password:  # Assuming format username:balance:password:score
                                self.app.user_score = int(user_data[3])  # Load user score
                                self.app.score_updated.emit(self.app.user_score)  # Emit signal to update score display
                                return True
        except Exception as e:
    ...
    def save_user(self, username, password):
        try:
            with open(USERS_FILE, 'a') as file:
                file.write(f"{username}:0:{password}:0\n")  # Save 0 balance and score on registration
        except Exception as e:
...
class MenuDialog(QDialog):
    def __init__(self, app, username):
        ...
        self.score_display = QLCDNumber(self)  # Create LCD number for score display
        self.score_display.setDigitCount(6)  # Set number of digits to display
        self.score_display.display(self.app.user_score)  # Initial value from the app
        ...
        layout.addRow(self.save_progress_button)
    ...
    def save_progress(self):
        self.app.save_user_progress()

    def update_score(self, score):
        self.score_display.display(score)  # Update the LCD number display
